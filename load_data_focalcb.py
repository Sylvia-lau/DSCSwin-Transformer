...
def run(i,boardname,model, num_epochs,numclasses,log_dir = '',test_li=[]):
    ...
    lr = 0.001
    optimizer = torch.optim.SGD(model.parameters(),  lr=lr, momentum=0.9, weight_decay=5e-4)    #,model.parameters(),[{'params':[ param for name, param in model.named_parameters() if 'patch' in name or 'ape' in name or 'pos_drop' in name or 'swin' in name or 'AFF' in name or 'downsamples' in name or 'norm' in name or 'head' in name]}],
    ...
    for epoch in range(num_epochs):
        train_loss,train_acc = train(epoch, model, lossFunction, optimizer,device, trainloader)
        test_loss,test_acc,test_pre_micro,test_rec_micro,test_f1_micro,PPV,NPV,TPR,TNR,FPR= test(model, lossFunction, device, validloader)
        scheduler.step()
