...
def load_custom_pretrained(model, default_cfg=None, load_fn=None, progress=False, check_hash=False) :
    ...
    model.load_pretrained(default_cfg['url'])
    # else :
...
def load_pretrained(model, default_cfg=None, num_classes=4, in_chans=3, filter_fn=None, strict=True,
                    progress=False) :
    ...
    if hf_hub_id and has_hf_hub(necessary=not pretrained_url) :
        ...
        state_dict = torch.load(pretrained_url)#load_state_dict_from_url(pretrained_url, progress=progress, map_location='cpu')
...
def build_model_with_cfg(
    ...
    num_classes_pretrained = 0 if features else getattr(model, 'num_classes', kwargs.get('num_classes', 4))
    if pretrained :
