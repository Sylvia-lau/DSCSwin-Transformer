...
from .layer.refiner_utils import DLA, ClassBlock
...
class Refiner_ViT(nn.Module) :
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, group=False, share_atten=False,
                 cos_reg=False,
        ...
        if isinstance(depth, int) :
            ...
            self.blocks = nn.ModuleList([
                Block(
                    dim=embed_dim, share=depth[i], num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                    qk_scale=qk_scale,
...
def Refiner_ViT_S(pretrained=False, **kwargs) :
    ...
    stage = [False, True] * 8
    model = Refiner_ViT(
        patch_size=16, embed_dim=384, depth=stage, apply_transform=apply_transform, num_heads=12, mlp_ratio=3,
        qkv_bias=True,
...
def Refiner_ViT_M(pretrained=False, **kwargs) :
    ...
    stage = [False, True] * 16
    model = Refiner_ViT(
        patch_size=16, embed_dim=420, depth=stage, apply_transform=apply_transform, num_heads=12, mlp_ratio=3,
        qkv_bias=True,
...
def Refiner_ViT_L(pretrained=False, **kwargs) :
    ...
    stage = [False, True] * 16
    model = Refiner_ViT(
        patch_size=16, embed_dim=512, depth=stage, apply_transform=apply_transform, num_heads=16, mlp_ratio=3,
        qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), share_atten=True, head_expand=3, **kwargs)
