...
from .layer.helpers import build_model_with_cfg, named_apply, adapt_input_conv
from .layer.drop import DropPath
from .layer.mlp import Mlp
from .layer.patch_embed import PatchEmbed
from .layer.weight_init import trunc_normal_,lecun_normal_
...
default_cfgs = {
    ...
    'vit_base_patch16_224_in21k': _cfg(
        url='/data/lyt/neck_region_transformer/weight/imagenet21k_ViT-B_16.npz',
        num_classes=4),
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.head_dist is not None:
            x, x_dist = self.head(feature[0]), self.head_dist(feature[1])  # x must be a tuple
            if self.training and not torch.jit.is_scripting():
                # during inference, return the average of both classifier predictions
                return x, x_dist
            else:
                return feature,(x + x_dist) / 2
        else:
            x = self.head(feature)
        return x
