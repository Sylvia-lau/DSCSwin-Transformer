...
# from .layer.deform_conv import DeformConv2d,DeformresBlock

_logger = logging.getLogger(__name__)

class SwinDown(nn.Module):
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.depthwise = nn.Conv2d(dim, dim, kernel_size=3, padding=1, stride=1, groups=dim)
    ...
    def forward(self, x):
        ...
        # conv_x = self.layer(conv_x)
        conv_x = self.depthwise(conv_x)
        conv_x = self.pointwise(conv_x)
        conv_x = conv_x.permute(0, 2, 3, 1).contiguous()
        conv_x = conv_x.view(B, -1, C)
        # conv_x = self.norm3(conv_x)
...
class Deformswinblockformer(nn.Module):
    r""" Swin Transformer
    ...
    def forward_features(self, x):
        ...
        x = self.pos_drop(x)
        x = self.layers(x)
        feature = self.norm(x)  # B L C
        x = self.avgpool(feature.transpose(1, 2))  # B C 1
        x = torch.flatten(x, 1)
        return x
