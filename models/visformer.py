...
from .layer.weight_init import trunc_normal_
from .layer.helpers import to_2tuple
...
class Visformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, init_channels=32, num_classes=6, embed_dim=384, depth=12,
...
if __name__ == '__main__':
    ...
    path = "./visformer_small_patch16_224.pth"
    parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print('number of parameters:{}'.format(parameters))
    for i in range(5):
        run(i,'./tensorboard_vis',model, 200, pth=path,test_acc_max_li=test_acc_max_li,test_loss_max_li=test_loss_max_li)
