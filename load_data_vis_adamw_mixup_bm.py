...
def train(epoch, model, lossFunction, optimizer, device, trainloader, mixup, scheduler, num_steps ):
...
def data_loader(k):
    ...
    trainset = torchvision.datasets.ImageFolder(root=r'/data/lyt/dataset/6c_1713_7-25_moreroi2/train/k' + str(k + 1),
                                                transform=transform_train)  # /data/lyt/dataset/neck_region_pre/6c_1382_new/train/k,D:\data\neck_region_pre(src)\train\k
    testset = torchvision.datasets.ImageFolder(root=r'/data/lyt/dataset/6c_1713_7-25_moreroi2/test/k' + str(k + 1),
...
def run(i,boardname,model, num_epochs,numclasses,log_dir = '',test_li=[],mixup=None):
    ...
    # lossFunction2 = nn.CrossEntropyLoss()    # test_acc_li,test_loss_li = [],[]
    ...
    for epoch in range(num_epochs):
        train_loss = train(epoch, model, lossFunction, optimizer, device, trainloader,mixup, scheduler, num_steps)
        test_loss, test_acc, test_pre_micro, test_rec_micro, test_f1_micro, PPV, NPV, TPR, TNR, FPR = test(model,lossFunction,device,testloader)
        scheduler.step()
